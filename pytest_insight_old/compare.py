...
class ComparisonAnalyzer:
    ...
    def compare_sessions(
        ...
        return {
            ...
            "performance_changes": ComparisonAnalyzer._analyze_performance_changes(base_results, target_results),
        }
    ...
    def compare_periods(
        ...
        period1_sessions = [s for s in sessions if period1_start <= s.session_start_time <= period1_end]
        period2_sessions = [s for s in sessions if period2_start <= s.session_start_time <= period2_end]

        return ComparisonAnalyzer._compare_session_groups(period1_sessions, period2_sessions)

    @staticmethod
    def _compare_session_groups(group1: List[TestSession], group2: List[TestSession]) -> Dict:
...
class SUTComparator:
    ...
    @staticmethod
    def compare_suts(sessions: List[TestSession], sut1: str, sut2: str, days: Optional[int] = None) -> Dict:
        ...
        return {
            ...
            "performance": SUTComparator._compare_performance(sut1_sessions, sut2_sessions, sut1_tests & sut2_tests),
            "stability": SUTComparator._compare_stability(sut1_sessions, sut2_sessions, sut1_tests & sut2_tests),
            "session_stats": {
    ...
    def _compare_performance(
        ...
        return {"significant_differences": sorted(differences, key=lambda x: abs(x[3]), reverse=True)[:10]}
    ...
    def _compare_stability(
        ...
        return {"stability_differences": sorted(differences, key=lambda x: abs(x[3]), reverse=True)}
