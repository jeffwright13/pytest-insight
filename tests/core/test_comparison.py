...
def test_compare_suts_with_sessions():
    tests_a = [TestResult(nodeid="test_foo", outcome=TestOutcome.PASSED, start_time=datetime(2025,4,21,10,0,0), duration=1.0)]
    tests_b = [TestResult(nodeid="test_bar", outcome=TestOutcome.FAILED, start_time=datetime(2025,4,21,10,0,0), duration=1.0)]
    sessions = [make_session("sutA", tests_a, "sess1"), make_session("sutB", tests_b, "sess2")]
    result = compare_suts(sessions, "sutA", "sutB")
    assert set(result.keys()) == {"sutA", "sutB"}
