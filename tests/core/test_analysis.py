...
def test_calculate_reliability_some_failed():
    tests1 = [TestResult(nodeid="test_foo", outcome=TestOutcome.PASSED, start_time=datetime(2025,4,21,10,0,0), duration=1.0)]
    tests2 = [TestResult(nodeid="test_foo", outcome=TestOutcome.FAILED, start_time=datetime(2025,4,21,11,0,0), duration=1.0)]
...
def test_calculate_reliability_multiple_tests():
    ...
    tests1 = [TestResult(nodeid="test_foo", outcome=TestOutcome.PASSED, start_time=t0, duration=1.0),
              TestResult(nodeid="test_bar", outcome=TestOutcome.FAILED, start_time=t0, duration=1.0)]
    tests2 = [TestResult(nodeid="test_foo", outcome=TestOutcome.PASSED, start_time=t1, duration=1.0),
