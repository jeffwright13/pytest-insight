...
def test_failure_trends():
    s1 = make_session('s1', [make_test_result("t4", "failed"), make_test_result("t5", "passed")], session_start_time=SimpleNamespace(date=lambda: "2023-01-01"))
    ti = TrendInsight([s1])
...
def test_insight_trend_and_dispatch(mocker):
    ...
    out2 = ti.insight(kind="summary")
    assert mock_summary.called
    out3 = ti.insight(kind="health")
    assert mock_summary.called
    # error
