...
def mock_profile_manager(temp_dir):
    ...
    mock_pm.get_profile.side_effect = lambda name=None: (
        test_profiles["test1"] if name is None else 
        test_profiles.get(name) or (lambda: ValueError(f"Profile '{name}' not found"))()
    )
...
def mock_get_storage_instance(mock_storage_instance):
    ...
    with mock.patch("pytest_insight.__main__.get_storage_instance", return_value=mock_storage_instance):
        yield
...
class TestAnalyzeCommands:
    ...
    def test_analyze_insights(
        ...
        result = runner.invoke(app, ["analyze", "insights"])
        assert result.exit_code == 0
        assert "Analyzing test results" in result.stdout

    def test_analyze_insights_with_profile(
        self, runner, mock_get_profile_manager, mock_get_storage_instance
    ):
        """Test analyzing insights with a specific profile."""
        result = runner.invoke(app, ["analyze", "insights", "--profile", "test1"])
        assert result.exit_code == 0
...
class TestMainCommand:
    ...
    def test_analyze_help(self, runner):
        ...
        assert result.exit_code == 0
        assert "Analyze test results" in result.stdout
