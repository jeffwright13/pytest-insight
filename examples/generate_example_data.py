...
class ExampleDataGenerator:
    ...
    def create_test_result(self, sut: str, base_time: datetime) -> TestResult:
        ...
        if properties["flaky"]:
            outcome = random.choices(["PASSED", "FAILED", "RERUN"], weights=[60, 20, 20])[0]
        else:
            outcome = random.choices(["PASSED", "FAILED", "SKIPPED"], weights=[85, 10, 5])[0]
    ...
    def create_session(self, sut: str, start_time: datetime) -> TestSession:
        ...
        results = [self.create_test_result(sut, start_time) for _ in range(num_tests)]

        # Calculate session duration based on test results
        max_test_end = max(r.start_time + timedelta(seconds=r.duration) for r in results)
        ...
        for day in range(num_days):
            ...
            for sut in self.suts:
                ...
                for _ in range(num_sessions):
                    session_time = day_date.replace(hour=random.randint(0, 23), minute=random.randint(0, 59))
