...
def analyze_test_data(
    ...
    if compare_with:
        if not json_mode:
            ...
            console.print("[yellow]Note: Comparison is now handled by the new Insights API.[/yellow]")
        ...
        if compare_type == "days":
            try:
                ...
                if not json_mode:
                    _format_rich_output(trend_data, title=f"Comparison with previous {days_to_compare} days")
                else:
...
def main():
    ...
    parser.add_argument(
        "--show-error-details", action="store_true", help="Show detailed error information"
    )
