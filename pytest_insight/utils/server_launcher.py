...
def run_high_level_api(host, port, reload):
    ...
    uvicorn.run("pytest_insight.rest_api.high_level_api:app", host=host, port=port, reload=reload)
...
def run_introspected_api(host, port, reload):
    ...
    uvicorn.run("pytest_insight.rest_api.introspective_api:introspected_app", host=host, port=port, reload=reload)


def main():
    """Launch the pytest-insight web interfaces."""
    parser = argparse.ArgumentParser(description="Run pytest-insight API servers")
    parser.add_argument("--host", default="127.0.0.1", help="Host to bind the servers to")
    parser.add_argument("--high-level-port", type=int, default=8000, help="Port for the high-level API server")
    parser.add_argument("--introspected-port", type=int, default=8001, help="Port for the introspected API server")
    parser.add_argument("--reload", action="store_true", help="Enable auto-reload on code changes")
    parser.add_argument("--db-path", type=str, default=None, help="Path to the test sessions database file")
    parser.add_argument("--high-level-only", action="store_true", help="Run only the high-level API server")
    parser.add_argument("--introspected-only", action="store_true", help="Run only the introspected API server")
    ...
    if run_high_level and run_introspected:
        ...
        print(f"Running Introspected API only: http://{args.host}:{args.introspected_port}")
    print("================================\n")
