...
def _create_dynamic_command(method_info: Dict[str, Any]):
    ...
    def command_func(
        ...
        **kwargs
        ...
        if output_format == OutputFormat.JSON:
            ...
            _format_rich_output(result, title=f"{component.capitalize()} {method_name.replace('_', ' ').title()}")
    ...
    for param_name, param_info in info["params"].items():
        ...
        if param_name in ["data_path", "sut_filter", "days", "test_pattern", "profile", "output_format"]:
            continue
