...
class ProfileManager:
    ...
    def __init__(self, profiles_path: Optional[Path] = None):
        ...
        Args:
            profiles_path (Optional[Path]): Optional path to profiles.json (for testing/isolation).
        """
        self._PROFILES_PATH = (
            Path(profiles_path) if profiles_path else Path.home() / ".pytest_insight" / "profiles.json"
        )
        self._LOCK_PATH = self._PROFILES_PATH.parent / "profiles.json.lock"
        self.profiles = {}
    ...
    def profiles_path(self) -> Path:
        """Public accessor for the profiles.json path (for testing)."""
        ...
        if self._PROFILES_PATH.exists():
            try:
                with open(self._PROFILES_PATH, "r") as f:
                    data = json.load(f)
                self.profiles = {k: StorageProfile.from_dict(v) for k, v in data.get("profiles", {}).items()}
                self.active_profile_name = data.get("active_profile")
            except Exception as e:
                print(f"[WARNING] Failed to load profiles from disk: {e}")
                self.profiles = {"default": StorageProfile(name="default")}
                    ...
                    json.dump(
                        {
                            ...
                            "modified_by": getpass.getuser() if hasattr(getpass, "getuser") else "unknown",
                        },
                        f,
                        indent=2,
                    )
        except Exception as e:
            print(f"[WARNING] Failed to save profiles to disk: {e}")
    ...
    def backup_profiles(self) -> Optional[Path]:
        ...
        backup_dir = self._PROFILES_PATH.parent / "backups"
        ...
        try:
            shutil.copy2(self._PROFILES_PATH, backup_path)
    ...
    def cleanup_backups(self, max_backups: int = 10) -> None:
        """Public method to clean up old backups, keeping only the newest N."""
        self._cleanup_old_backups(max_backups=max_backups)
    ...
    def restore_from_backup(self, backup_path: Union[str, Path]) -> bool:
        ...
        try:
            ...
            shutil.copy2(backup_path, self._PROFILES_PATH)
...
class JSONStorage(BaseStorage):
    ...
    def _write_json_safely(self, sessions_data: List[Dict]) -> None:
        ...
        try:
            with lock:
                temp_file = tempfile.NamedTemporaryFile(mode="w", delete=False, dir=str(self.file_path.parent))
                try:
                    json.dump(sessions_data, temp_file, indent=2)
                    temp_file.close()
    ...
    def load_sessions(
        ...
        **kwargs,
        ...
        """
        Load all test sessions from storage. Always reloads from disk.
        Args:
        ...
        if use_streaming:
            ...
            try:
                ...
                return []
        # Standard read
        sessions_data = self._read_json_safely()
        sessions = []
    ...
    def save_sessions(self, sessions: List[TestSession]) -> None:
        """
        Save multiple test sessions to storage. Always writes immediately and atomically.
        Args:
    ...
    def save_session(self, session: TestSession) -> None:
        """
        Save a single test session to storage. Always reloads first, then writes immediately.
        Args:
            session (TestSession): Test session to save
        """
        try:
    ...
    def get_session_by_id(self, session_id: str) -> Optional[TestSession]:
        """
        Get a test session by its ID. Always reloads from disk.
        Args:
            session_id (str): The ID of the session to retrieve
        Returns:
            TestSession: The TestSession with the matching ID or None if not found
    ...
    def clear(self) -> None:
        """
        Clear all sessions from storage. Writes immediately.
        Returns:
            None
        """
        self._write_json_safely([])
...
def get_storage_instance(
    ...
    if profile_name:
        try:
            ...
            profile = profile_manager.create_profile(profile_name)
        ...
        if profile.storage_type.lower() == "json":
            ...
            raise ValueError(f"Unsupported storage type for profile '{profile_name}': {profile.storage_type}")
...
def get_profile_manager(force_reload=False) -> ProfileManager:
    """
    Get the global profile manager instance.

    Args:
        force_reload (bool): Whether to force reload the profiles from disk
    Returns:
...
def create_profile(name: str, storage_type: str = "json", file_path: Optional[str] = None) -> StorageProfile:
    ...
    profile = profile_manager.create_profile(name, storage_type, file_path)
    return profile
...
def get_profile_metadata(name: Optional[str] = None) -> Dict[str, Any]:
    ...
    if not profile_manager._PROFILES_PATH.exists():
        return {"error": "No profiles configuration found"}

    try:
        data = json.loads(profile_manager._PROFILES_PATH.read_text())
