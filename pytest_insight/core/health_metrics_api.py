"""Health metrics API for pytest-insight.

This module provides a clean API for accessing health metrics in pytest-insight.
It serves as a bridge between the Analysis class and the health metrics implementations.
"""


class HealthMetricsAPI:
    """API for accessing health metrics in pytest-insight.

    This class provides methods for calculating various health metrics
    for test sessions, including:
    - Top failing tests
    - Regression rate
    - Longest running tests
    - Test suite duration trend

    It can be used directly or as a mixin for other classes.
    """

    def __init__(self, sessions=None):
        """Initialize the health metrics API.

        Args:
            sessions: Optional list of test sessions to analyze
        """
        self._sessions = sessions or []

    def _get_sessions(self, days=None):
        """Get sessions filtered by days.
        ...
        """
        if not hasattr(self, "_filter_sessions_by_days"):
            # If we're being used as a standalone class
            return self._sessions

        # If we're being used as a mixin
        return self._filter_sessions_by_days(days)
    ...
    def regression_rate(self, days=None):
        ...
        sorted_sessions = sorted(
            ...
            key=lambda s: (
                ...
                if hasattr(s, "session_start_time")
                else datetime.min
            ),
        )
        ...
        for nodeid, history in test_history.items():
            ...
            if (
                last_passing_idx is not None
                and last_passing_idx < len(history) - 1
                and history[-1]["outcome"] == TestOutcome.FAILED
            ):
    ...
    def longest_running_tests(self, days=None, limit=10):
        ...
        sorted_results = sorted(results, key=lambda x: x["avg_duration"], reverse=True)

        return {"longest_tests": sorted_results[:limit]}
