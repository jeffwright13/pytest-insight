...
v1_analysis_path = os.path.join(
    os.path.dirname(__file__), "../pytest_insight_v1/core/analysis.py"
)
spec = importlib.util.spec_from_file_location(
    "v1_analysis", os.path.abspath(v1_analysis_path)
)
...
def print_sessions(sessions):
    ...
    for s in sessions:
        table.add_row(
            ...
            str(getattr(s, "sut", "?")),
            str(getattr(s, "session_start_time", "?")),
            str(len(getattr(s, "test_results", []))),
        )
    console.print(table)
...
def print_flakiness(flakiness):
    ...
    for test_name, result in flakiness.items():
        table.add_row(
            test_name, str(result["flaky"]), str(result["passes"]), str(result["fails"])
        )
    console.print(table)
...
def main():
    ...
    console.print(
        Panel(
            "[bold cyan]pytest-insight Fluent CLI Explorer[/bold cyan]\nType 'help' for commands. Type 'exit' to quit.",
            expand=False,
        )
    )
    while True:
...
"""
                ...
                console.print(
                    f"Current query: [green]{context.get('query_str','(none)')}[/green]"
                )
            elif cmd == "list_suts":
                ...
                if not sessions:
                    console.print(
                        "[yellow]No sessions loaded. Run a query first.[/yellow]"
                    )
                else:
                    suts = sorted(
                        set(
                            getattr(s, "sut_name", None)
                            for s in sessions
                            if hasattr(s, "sut_name")
                        )
                    )
                    console.print(
                        f"[bold cyan]Available SUTs:[/bold cyan] {', '.join(suts) if suts else '[none]'}"
                    )
            elif (
                cmd.startswith("query.")
                or cmd.startswith("in_last_days(")
                or cmd.startswith("filter_by_test()")
                or cmd.startswith("with_outcome(")
                or cmd.startswith("apply()")
                ...
                try:
                    ...
                    if cmd.startswith("query."):
                        ...
                        context["query_str"] = (
                            context.get("query_str", "query") + "." + cmd
                        )
                ...
                try:
                    ...
                    if not sessions:
                        console.print(
                            "[yellow]No sessions to analyze. Run a query first.[/yellow]"
                        )
                        continue
                ...
                try:
                    ...
                    console.print(
                        "[yellow]Error patterns insights are not available in this version.[/yellow]"
                    )
                except Exception as e:
                    console.print(
                        f"[red]Error running error pattern insights:[/red] {e}"
                    )
            else:
