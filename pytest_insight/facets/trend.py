...
class TrendInsight:
    ...
    def emerging_patterns(self):
        ...
        for s in self.sessions:
            for t in s.test_results:
                test_history[t.nodeid].append(
                    (getattr(s, "session_start_time", None), t.outcome, t.duration)
                )
        for nodeid, records in test_history.items():
            ...
            if len(durations) > 5 and max(durations) > 2 * sum(durations) / len(
                durations
            ):
                patterns.append(
                    {
    ...
    def failure_trends(self) -> dict:
        ...
        trends = {
            d: {"fail_rate": v["failed"] / v["total"] if v["total"] else 0.0, **v}
    ...
    def unified_insight(self, kind: str = "trend", tabular: bool = True):
        ...
        if kind == "trend":
            ...
            if tabular:
                ...
                return tabulate(
                    rows, headers=["NodeID", "Issue", "Detail"], tablefmt="github"
                )
            else:
    ...
    def as_dict(self):
        ...
        return {
            ...
            "failure_trends": self.failure_trends(),
            "emerging_patterns": (
                self.emerging_patterns() if hasattr(self, "emerging_patterns") else None
            ),
        }
    ...
    def filter(self, sut: str = None, nodeid: str = None):
        ...
        if sut is not None:
            filtered_sessions = [
                s for s in filtered_sessions if getattr(s, "sut_name", None) == sut
            ]
