...
def get_config_values(config, keys):
    """
    For each key, determine value by:
    1. Command-line option (if present)
    2. Environment variable (all-caps, prefixed with 'PYTEST_INSIGHT_')
    3. Hardcoded default (from the keys dict)
    Returns a dict of resolved config values.
    """
    resolved = {}
    for key, default in keys.items():
        cli_val = config.getoption(key, None)
        env_var = f"PYTEST_INSIGHT_{key.upper()}"
        env_val = os.environ.get(env_var)
        resolved[key] = cli_val or env_val or default
    return resolved
...
def pytest_configure(config: Config):
    ...
    try:
        ...
        try:
            profile_manager.get_profile(config_values["insight_profile"])
        except ValueError:
            # Profile doesn't exist, create it
            create_profile(config_values["insight_profile"], "json", None)  # None will use the default path
            print(
                f"[pytest-insight] Created new profile '{config_values["insight_profile"]}'",
                file=sys.stderr,
            )

        # Now get the storage instance using the profile
        storage = get_storage_instance(profile_name=config_values["insight_profile"])
